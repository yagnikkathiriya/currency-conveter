...
class ApiService {
  Future<List<ModelApi>> getLatest(String baseCurrency) async {
    String url = '${base_url}apikey=${"cur_live_eC3mPMUHZKMh8HmfFoMLZgqvqnUZthkuLy1hVddW"}&base_currency=$baseCurrency';
  ...
  Future<List<ModelApi>> getExchange(String baseCurrency,String targetCurrency) async {
    String url = '${base_url}apikey=${"cur_live_eC3mPMUHZKMh8HmfFoMLZgqvqnUZthkuLy1hVddW"}&base_currency=$baseCurrency&currencies=$targetCurrency';
    List<ModelApi> currencyList = [];
    try {
      final response = await get(Uri.parse(url));
      if (response.statusCode == 200) {
